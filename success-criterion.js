class SuccessCriterion extends HTMLElement {
    ...
    connectedCallback() {
        this.fetchCriteria().then(() => {
            ...
            console.log('before render', this.criterion)
            this.render();
        });
    }
    ...
    render() {
        ...
        [hidden] { display: none !important; }
         ...
         :host([data-mode="simple"]), :host([data-mode="tiny"]) {
             border: none;
             padding: 0;
             .sc-info *:not(.sc--simple, .sc--simple *) {
                // display: none;
             }
         }
         :host([data-mode="tiny"]) {
            .sc-info *:not(.sc--tiny, svg *) {
                // display: none;
            }
            
         }
        `;
        const select = criteria => 
        `<select aria-label="Select a success criterion">
            <option value="">Select a success criterion</option>
            ${criteria.map((criterion) => 
            `<option 
                value="${criterion.ref_id}" 
                ${criterion.ref_id === this?.criterion?.ref_id ? "selected" : ""}>
        ...
        ${this.criterion && !['tiny', 'simple'].includes(this.mode) ?
        `<div class="sc-detailed">
        <span class="sc-title">
            ${this.criterion.ref_id} - ${this.criterion.title}
            <span class="sc-level">${this.criterion.level}</span>
